...
async def run(playwright: Playwright) -> None:
    ...
    # 从 文件中读取订货会列表
    with open("meetings_celine.json", "r", encoding="utf-8") as f:
...
async def process_collections(browser, meeting, semaphore):
    ...
    async with semaphore:
        ...
        brand = meeting.get("attr_data", {}).get("brand_name")
...
async def parse_api(resp: dict, *, nav_id: str, gender_id: str,
    ...
    if resp.get("status_code") == 200:
        ...
        for meeting in images_list:
            ...
            has_drawing = meeting.get("has_drawing", False)
            drawings = []
            if has_drawing:
                drawing_images = meeting.get("drawing", [])
