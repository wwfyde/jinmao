...
async def open_pdp_page(
    ...
    async with semaphore:
        ...
        # 临时储存商品信息目录
        product_folder = f"projects/jcpenney/products"
        ...
        if Path(product_file).exists():
            logger.info(f"已经抓取过产品 {product_id=}")
            need_crawl = False
        ...
        if Path(reviews_file).exists():
            logger.info(f"已经抓取过评论 {product_id=}")
            need_review = False
        ...
        async def handle_route(route: Route):
            ...
            if "reviews.json" in request.url:
                ...
                logger.info(f"预期评论数{total_count}, 总页数{total_pages}")
        ...
        try:
            ...
            logger.info(f"已经保存产品数据 {product_id=}")
            ...
            if review_button:
                if need_review:
                    ...
                    logger.info("点击查看评论按钮完成")
                    await page.wait_for_timeout(8000)

            await route_event.wait()
            await page.wait_for_timeout(3000)
            logger.info(f"评论抓取完成 {product_id=}")
            logger.info(f"关闭页面 {product_id=}")
        except TimeoutError:
            pass
        except PlaywrightTimeoutError:
...
async def main():
    ...
    for f in Path("projects/jcpenney/clean_data").glob("wome*.json"):
        with open(f) as file:
            logger.info("-" * 50)
            logger.info(f"开始处理: {f}")
            urls = json.load(file)
            await run(urls)
    logger.info(f"耗时{time.perf_counter() - start}")
