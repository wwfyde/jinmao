...
file_path = r"data/test/gap_coat"
img_detail2 = []
...
def run(playwright: Playwright) -> None:
    ...
    for i in range(1, 115):
        page.locator(
            "#search-page > div > div > div > div.search-page__product-grid.css-1xlfwl6 > section > div > div:nth-child({})".format(
                i
            )
        ).first.click()
        ...
        for j in range(1, 10):
            try:
                detail = tree.xpath(
                    '//*[@id="buy-box-wrapper-id"]/div/div[2]/div/div/div/div[2]/div[2]/div/div[1]/div/ul/li[{}]/span/text()'.format(
                        j
                    )
                )[0]
                # log.info(detail)
