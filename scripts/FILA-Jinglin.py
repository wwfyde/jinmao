    ...
    for i in range(1, 16):
        for j in range(1, 5):
            with page.expect_popup() as page1_info:
                page.locator(
                    "#J_ShopSearchResult > div > div.J_TItems > div:nth-child({}) > dl:nth-child({}) > dt".format(i, j)
                ).first.click()
            ...
            for y in range(1, 26):
                try:
                    img_url = tree.xpath(
                        '//*[@id="root"]/div/div[2]/div[2]/div[1]/div/div[2]/div[5]/div/div/div[1]/div[1]/div/div[{}]/div/img/@src'.format(
                            y
                        )
                    )[0]
                    # img_url = tree.xpath(
