...
summary_prompt = "You are an e-commerce and sentiment analysis specialist. Based on the following customer feedback analyses, provide a comprehensive summary analysis of the product. Mention overall product quality, comfort, and other key aspects that are frequently mentioned. Format your response as a paragraph summarizing the general sentiment and key takeaways."
...
def summarize_reviews(analyses):
    ...
    stream = client.chat.completions.create(
        model="ep-20240611063733-gdf2s",  # 您的模型端点ID
        messages=[{"role": "system", "content": summary_prompt}, {"role": "user", "content": summary_content}],
        stream=True,
    )
